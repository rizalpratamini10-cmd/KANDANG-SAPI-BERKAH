...
if($_SERVER['REQUEST_METHOD'] == 'POST') {
    ...
    $update = "UPDATE metode_pembayaran SET 
               bank_name = '$bank_name',
    ...
    if(mysqli_query($conn, $update)) {
        ...
        // Refresh data
        $result = mysqli_query($conn, "SELECT * FROM metode_pembayaran WHERE id = $id");
        $bank = mysqli_fetch_assoc($result);
    } else {
        ...
        <div class="form-card">
            ...
            <form method="POST" action="">
                ...
                <div class="form-group">
                    ...
                    <input type="text" name="account_number" value="<?php echo $bank['account_number']; ?>" required>
                </div>
                
                <div class="form-group">
                    <label>Atas Nama *</label>
                    <input type="text" name="account_holder" value="<?php echo $bank['account_holder']; ?>" required>
                </div>
                
                <div class="form-group checkbox-group">
                    <input type="checkbox" name="is_active" id="is_active" <?php echo $bank['is_active'] ? 'checked' : ''; ?>>
                    <label for="is_active">Aktifkan rekening ini</label>
                </div>
                
                <button type="submit" class="btn-save">Update Rekening</button>
                <a href="metode-pembayaran.php" style="margin-left: 10px; color: #666;">Kembali</a>
            </form>
        </div>
    </div>
</div>
